...
则将$z_j$分裂为两个新的聚类中心且$N_c$加1。$z_j^+=z_j+\sigma_{j_{max}},z_j^-=z_j-\sigma_{j_{max}}$。如果本步骤完成了分裂运算,则转至第二步,否则继续。
...
> 
> 算法复杂度较高,为$O(N^2T)$,$N$为样本数,$T$为迭代次数,而K-Means只是$O(N*K)$的复杂度。因此当N比较大时(N>3000),AP聚类算法往往需要算很久。
> 
