...
**最大可分性:** 样本点$x_i$在新空间上的投影为$W^Tx_i$,若所有样本点能近可能分开,则应该使投影后样本的方差最大,即$\sum_ix_i^TWW^Tx_i$,由$x_i^TWW^Tx_i=tr(W^Tx_ix_i^TW)$,可知优化目标函数为
...
IncrementalPCA 类为了增量式的更新explainedvariance_ratio,仅需要存储估计出的分量和噪声方差。 
