...
下面是一段 nms 的代码，参考自：Faster Non-Maximum Suppression for Object Detection，详情见注释。
...
上面这段代码没有指定每个框的置信度，所以使用的是框的右下角的 y 坐标来代替。
...
https://blog.csdn.net/u011534057/article/details/51235718



soft_nms
这是对 nms 的改进，它没有删除所有IOU大于阈值的框，而是降低其置信度。
...
[公式] 表示阈值为 i 时的某一连通区域，Δ 为灰度阈值的微小增加量， [公式] 为阈值是 i 时的区域 [公式] 的变化率， [公式] 表示区域 [公式] 的面积。当 [公式] 为局部极小值时，说明 [公式] 区域的变化非常小，则 [公式] 为可以被认为是最大稳定极值区域。

其实mser的数学原理比这个复杂，详情可以查看下面的参考文章。
...
使用 mser = cv2.MSER_create() 来得到一个mser对象，cv2.MSER_create() 的参数如下：

_delta   it compares (size_i − size_{i−delta}) / size_{i−delta}, default 5
...
_delta: 允许灰度阈值的最小变化步长，即一次灰度值增加的量，取值范围（0,160），默认为5
_min_area: 允许区域最小的面积，默认为60
_max_area: 允许区域最大的面积，默认为14400
_max_variation: 允许不同强度阈值下的区域之间的最大面积变化率，取值范围为(0,1)，越接近1，则越多区域被认为是稳定的，如果接近0，则只能找到非常少的稳定区域，默认为0.25


使用 regions, boxes = mser.detectRegions(gray) 来获取mser区域。输入图像可以是灰度图或者灰度图的梯度图，regions 和 boxes 都是 mser 区域，不过形式不同。

regions是一个列表，其中每个元素都表示一个区域，每个区域为表示该区域的所有像素点的坐标的集合，每个区域的尺寸为(n_points, 2)，区域的面积就是这些点的个数。
boxes是一个二维数组，尺寸为(n_regions, 4)，每一行表示一个区域的信息：(x,y,w,h)。


获取 mser 区域后，可以进行如下后续处理：

之后可以结合角点检测将筛选出准确的文字区域；
根据角度，长宽比等几何关系剔除较明显的非文字区域；
对mser区域的二值图进行腐蚀膨胀等操作，得到文字区域的连通域，然后获取其轮廓；
使用非极大值抑制获取不重复的区域。


下面是一个使用 mser 进行文字检测的例子（需要用到上面的非极大值抑制）：
...
plt.show()
结果如下，下面三张图分别为，对区域绘制不规则轮廓，绘制矩形轮廓，应用非极大值抑制后的矩形轮廓。




得到文字区域后，可以单独提取出文字区域部分：
...
得到的text_mask和text_region分别如下：




可以看到我们不仅得到文字区域，还得到了纹理复杂的背景区域，这是使用 mser 方法不可避免的，因为使用的是类似于分水岭的水淹法，这时可以通过笔画宽度变换（Stroke Width Transform，SWT）只得到其中的文字部分。

下次讲讲 Stroke Width Transform 吧。



参考文章：

Tutorials > MSER feature detector
...
https://www.cnblogs.com/frischzenger/p/3334569.html
